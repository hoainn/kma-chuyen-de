...
\subsection{Framework tham chiếu DeSFAM và ba mô-đun}
\label{subsec:desfam_modules}
DeSFAM (\emph{Dynamic eBPF-Driven Syscall Filtering and Anomaly Mitigation})~\cite{desfam_original}
là một framework bảo mật thời gian thực, nhẹ cho container, gồm \emph{ba mô-đun}
(giai đoạn) tích hợp:
\begin{enumerate}[label=\textbf{Mô-đun \arabic*.},leftmargin=2.4em]
  \item \textbf{Lập danh sách truy cập syscall lai và sinh hồ sơ seccomp.} Kết hợp
        phân tích tĩnh (đồ thị lời gọi từ binary ELF) với lập hồ sơ động (eBPF dưới
        khối lượng công việc chuẩn) và lọc theo thông tin CVE để sinh hồ sơ seccomp
        tối thiểu. \emph{Chỉ điểm qua trong báo cáo này.}
  \item \textbf{SyscallAD---giám sát thời gian thực và phát hiện bất thường.} Phân
        tích chuỗi syscall theo cửa sổ trượt bằng tập hợp VAE\,+\,Isolation Forest
        (kèm khai phá mẫu lành tính PrefixSpan và chọn ngưỡng) để phát hiện leo
        thang đặc quyền, thoát container và chèn syscall---kể cả khi lạm dụng các
        syscall được phép. \textbf{Đây là mô-đun trọng tâm của chuyên đề.}
  \item \textbf{Thực thi thích ứng và quản lý chính sách.} Chấm điểm rủi ro (ánh xạ
        MITRE ATT\&CK, mức nghiêm trọng tích lũy, khớp mẫu khai thác) rồi
        chặn/giới hạn tốc độ/ghi nhật ký trong nhân bằng eBPF gắn vào LSM hook.
        \emph{Chỉ điểm qua trong báo cáo này.}
\end{enumerate}
Chuyên đề nghiên cứu sâu \textbf{Mô-đun~2 (SyscallAD)}: hệ thống hóa thiết kế, hiện
thực hóa lõi phát hiện và đánh giá thực nghiệm; hai mô-đun còn lại được trình bày
vắn tắt ở trên chỉ nhằm định vị SyscallAD trong tổng thể framework.
