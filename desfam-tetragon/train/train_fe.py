...
class RobustScaler:
    """Quantile-based normalizer (p1..p99) fitted on a reference set.
    Robust to outliers — VAE reconstruction errors can span 6 orders of
    magnitude, which collapses a plain min-max scaler. Using p1/p99 keeps
    the bulk of the distribution mapped to [0, 1] while clipping extremes."""
    def __init__(self, ref_scores, lo_pct=1.0, hi_pct=99.0):
        self.lo = float(np.percentile(ref_scores, lo_pct))
        self.hi = float(np.percentile(ref_scores, hi_pct))
...
norm_if = RobustScaler(if_scores_train)
...
# Normalizer fitted on training (same reference for val + test)
norm_vae = RobustScaler(vae_scores_train)
...
# ─────────────────────────────────────────────────────────────────────────────
print('\n' + '='*60)
print('ENSEMBLE  (α=0.7·VAE + 0.3·IF, training-fitted p1/p99 RobustScaler)')
print('='*60)
ALPHA = 0.7
...
report = {
    ...
    'models': {
        ...
        'Ensemble': {
            'params': {'alpha': ALPHA, 'normalization': 'Robust_p1_p99_on_training_scores'},
